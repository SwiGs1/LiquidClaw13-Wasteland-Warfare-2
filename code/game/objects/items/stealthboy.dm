...
/obj/item/stealthboy/malfuctioning
	name = "Malfuctioning Steath Boy"
	desc = "This Stealth Boy 3001 has sustained damage as well as wear and tear from overuse. It has less power and will deplete faster"
	icon = 'icons/obj/device.dmi'
	icon_state = "stealthboy"
	item_flags = NOBLUDGEON
	slot_flags = ITEM_SLOT_BELT
...
/obj/item/stealthboy/malfuctioning/item_action_slot_check(slot, mob/user)
	if(slot == SLOT_BELT)
		return 1
...
/obj/item/stealthboy/malfuctioning/dropped(mob/user)
	..()
	if(user && user.get_item_by_slot(SLOT_BELT) != src)
		Deactivate()

/obj/item/stealthboy/malfuctioning/process()
	if(user.get_item_by_slot(SLOT_BELT) != src)
		Deactivate()
		return
	if(on)
		animate(user,alpha = CLAMP(255 - charge,0,255),time = 10)
		charge = max(0,charge - 13)
